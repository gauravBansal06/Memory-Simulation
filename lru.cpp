...
{
    init();  //initializing
    vector<int> lru_check(P,0); //to get least recently used page
    int curr_proc,curr_vp,rep_proc,rep_vp,lru_page;
    for(int i=0; i < page_ref.size(); i++){
        ...
        if(pagetable[curr_proc][curr_vp] != -1){  // if hit
            ...
            lru_check[pagetable[curr_proc][curr_vp]] = i;  //updating lru value of index in memory
        }
        else{ //if miss
            ...
            if(memory.size() < P){ //if memory is not full
                ...
                lru_check[memory.size()] = i;
                memory.push_back({curr_proc,curr_vp});
            }
            else{
                lru_page = min_element(lru_check.begin(),lru_check.end()) - lru_check.begin();
                rep_proc = memory[lru_page].first;
                rep_vp = memory[lru_page].second;
                pagetable[curr_proc][curr_vp] = lru_page;  //replacing page in memory
                pagetable[rep_proc][rep_vp] = -1;  //removing page from memory
                memory[lru_page].first = curr_proc;
                memory[lru_page].second = curr_vp;
                lru_check[lru_page] = i;
            }
...
void local_page_replacement()
{
    ...
    for(int i=0; i < page_ref.size(); i++){
        ...
        if(pagetable[curr_proc][curr_vp] != -1){  // if hit
            ...
            overall_hit++;  //overall hits
            lru_check[pagetable[curr_proc][curr_vp]] = i;  //updating lru value of index in memory
            lru_proc[curr_proc].erase(find(lru_proc[curr_proc].begin(),lru_proc[curr_proc].end(),pagetable[curr_proc][curr_vp]));
            lru_proc[curr_proc].push_back(pagetable[curr_proc][curr_vp]); //updating lru value of current page 
        }
        else{ //if miss
            cout << "Page fault for process = " << curr_proc << " and virtual page = " << curr_vp << ". Showing page table: " << endl;
            cout << "--------------------------------------------------------------------" << endl;
            ...
            if(memory.size() < P){ //if memory is not full
                pagetable[curr_proc][curr_vp] = memory.size();
                lru_check[memory.size()] = i;
                lru_proc[curr_proc].push_back(memory.size());
                memory.push_back({curr_proc,curr_vp});
            }
            else{
                if(lru_proc[curr_proc].size() == 0){
                    lru_page = min_element(lru_check.begin(),lru_check.end()) - lru_check.begin();
                }
                else{
                    lru_page = lru_proc[curr_proc][0];
                }
                rep_proc = memory[lru_page].first;
                rep_vp = memory[lru_page].second;
                pagetable[curr_proc][curr_vp] = lru_page;  //replacing page in memory
                ...
                memory[lru_page].second = curr_vp;
                lru_check[lru_page] = i;
                lru_proc[rep_proc].erase(lru_proc[rep_proc].begin());
                lru_proc[curr_proc].push_back(lru_page);
            }
            cout << "After Fault:" << endl;
            print(curr_proc);  //printing page table
            for(int o = 0; o < 2; o++){
